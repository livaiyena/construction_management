...
router.get('/project-details/:projectId', async (req, res) => {
    try {
        ...
        res.json({ data: result.rows[0] || {}, query: sqlQuery.trim() });
    } catch (error) {
        console.error('SQL Query Error:', error);
        res.status(500).json({ message: 'Sorgu hatası', error: error.message });
    }
    ...
    try {
        const sqlQuery = `
            SELECT 
                ...
                AVG(e.daily_rate) AS avg_daily_rate,
                MIN(e.daily_rate) AS min_daily_rate,
                MAX(e.daily_rate) AS max_daily_rate,
                ROUND(AVG(e.daily_rate) * 30, 2) AS estimated_monthly_avg
            FROM "Roles" r
            LEFT JOIN "Employees" e ON r.id = e."RoleId"
            GROUP BY r.id, r.name
            ORDER BY avg_daily_rate DESC NULLS LAST
        `;
...
router.get('/pending-expenses', async (req, res) => {
    try {
        const sqlQuery = `
            ...
            WHERE CURRENT_DATE - e.expense_date <= 45
