...
export default function Employees() {
    ...
    const fetchInitialData = async () => {
        setIsLoading(true)
        try {
            // 3 farkl─▒ veriyi paralel ├ğekiyoruz: ├çal─▒┼şanlar, Projeler, Roller
    ...
    const handleAddRole = async () => {
        ...
        try {
            ...
            setFormData({ ...formData, RoleId: res.data.id }); // Formda otomatik se├ğ
            setNewRoleName('');
            setShowRoleInput(false);
            showToast('Yeni görev tanımı eklendi.', 'success');
        } catch (error) {
            showToast('Bu g├Ârev zaten mevcut olabilir.', 'warning');
        }
    }
    // ----------------------------------
    ...
    const handleSubmit = async (e) => {
        ...
        // Telefon numaras─▒n─▒ temizle (sadece rakam ve + kals─▒n)
        const cleanPhone = formData.phone.replace(/\s/g, '')

        // Validasyon: RoleId ve ProjectId bo┼şsa null g├Ânderilmeli (Veritaban─▒ ili┼şkisi i├ğin)
        const payload = {
            ...formData,
            phone: cleanPhone,
                                ...
                                employees.filter(emp => {
                                    ...
                                    >
                                        ...
                                        <td className="px-6 py-4 text-right font-medium text-slate-700">
                                            {emp.daily_rate ? `Ôé║${Number(emp.daily_rate).toLocaleString('tr-TR')}` : '-'}
                                        </td>
                                        ...
                                        <input 
                                            ...
                                            onChange={e => {
                                                ...
                                                if (input.startsWith('+')) {
                                                    ...
                                                    formatted = '+'
                                                    if (digits.length > 0) formatted += digits.substring(0, 2) // ├£lke kodu: 90
                                                    if (digits.length >= 3) formatted += ' ' + digits.substring(2, 5) // ─░lk 3 rakam
                                                    if (digits.length >= 6) formatted += ' ' + digits.substring(5, 8) // ─░kinci 3 rakam
