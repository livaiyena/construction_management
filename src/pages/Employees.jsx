    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (isEditing) {
                ...
                addNotification('success', `Personel güncellendi: ${formData.first_name} ${formData.last_name}`, 'EMPLOYEE')
            } else {
                await api.post('/employees', payload)
                showToast('Personel eklendi.', 'success')
                addNotification('success', `Yeni personel eklendi: ${formData.first_name} ${formData.last_name}`, 'EMPLOYEE')
            }
            fetchInitialData()
            setShowModal(false)
            resetForm()
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Bu çalışanı silmek istediğinize emin misiniz?')) {
            try {
                ...
                addNotification('warning', `Personel silindi. (ID: ${id})`, 'EMPLOYEE')
            } catch (err) {
